...
public class LoadSounds implements Runnable {
	...
	/**
	 * Load all files in this directory in as sound clips
	 * Preallocate an internal buffer big enough for the biggest one
	 *
	 */
	...
	/**
	 * Load up all the ogg files in one directory one by one
	 * to do this faster, save this objects as a single database and then
	 * load
	 */
	public void load(String dirName) {
		...
		File[] files = dir.listFiles(new FileFilter("*.ogg"));
		int totalSize = 0;
		for (File f : files)
			totalSize += f.size();
		sounds = new byte[totalSize];
		int offset = 0;
		for (int i = 0; i < files.length; offset +=files[i].size(), i++) {
			load(files[i], offset);
		}
	}
	 ...
	 */
	private byte[] load(File f, int offset) throw Exception {
		long size = f.size();
	...
	/**
	 * Initializes JOrbis. 
	 *  1. Initialize the <code>SyncState</code> object.
	 *  2. Prepare the <code>SyncState</code> buffer.
	 *  3. initialize the buffer, taking the data in <code>SyncState</code>.
	 */
	private void initializeJOrbis() {
		//		debugOutput("Initializing JOrbis.");
		joggSyncState.init();		          // Initialize SyncState
		joggSyncState.buffer(bufferSize); // Prepare to SyncState internal buffer
		buffer = joggSyncState.data; // fill the buffer from SyncState's internal
		// buffer.
	...
	private boolean readHeader() {
		...
		while(needMoreData) {	// Read from the InputStream.
			int count = inputStream.read(buffer, index, bufferSize);
